import { Image, ImageProps } from 'antd';
import React, { useEffect, useState } from 'react';
import ImageCustom from 'src/components/Custom/Image';

interface AntdImageProps extends ImageProps {
  linkImage: string;
  className?: string;
}

const AntdImage = (props: AntdImageProps) => {
  const { linkImage, className, ...rest } = props;
  const [isError, setIsError] = useState<boolean>(false);
  useEffect(() => {
    setIsError(false);
  }, [linkImage]);

  return (
    <>
      {isError ? (
        <ImageCustom
          src={
            'data:image/png;base64,'
          }
          className={className}
        />
      ) : (
        <Image
          onError={() => {
            setIsError(true);
          }}
          preview={false}
          className={className}
          src={linkImage}
          {...rest}
        />
      )}
    </>
  );
};

export default AntdImage;
